...
def solicitar_exames(request):
    if request.method == 'GET':
        ...
        #TODO: Caulcular preco dos dados disponiveis
        preco_total= 0
        
        for i in solicitacao_exame:
            if i.disponivel:
                preco_total += i.preco
            print(preco_total)
...
def fechar_pedido(request):
    ...
    messages.add_message(request, constants.SUCCESS, 'Pedido de exame concluido com sucesso')
...
def cancelar_pedido(request, pedido_id):
    ...
    #verificação de segurança: alteração de dados de outro usuario=integridade
